...
export const CourseBlock = ({ menu, className, ...props }: CourseBlockProps) => {
	...
	return (
		<div className={cn(className)} {...props}>
			<Card className={styles.category} key={menu._id.secondCategory}>
				<Htag className={styles.courseName} tag="h3">{menu._id.secondCategory}</Htag>
				<div className={styles.courses}>
					{isFullOpened &&
						menu.pages.map((p) => (
							<Link className={styles.courseLink} key={p._id} href={`/courses/${p.alias}`}>
								{p.title}
							</Link>
						))}
					{!isFullOpened &&
